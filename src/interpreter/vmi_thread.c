...
const vm_byte* _vmi_thread_loadx(vmi_thread* t, const vm_byte* ip)
{
	...
	if (block == NULL)
		return _vmi_thread_stack_out_of_memory(t, ip);
	block->value = t->locals[instr->block_index].value;
	return ip + sizeof(vmi_instr_loadx);
}

inline const vm_byte* _vmi_thread_loadl(vmi_thread* t, const vm_byte* ip, const vm_uint32 block_index)
{
	vmi_stack_block* const block = vmi_stack_push(&t->stack, 1);
	if (block == NULL)
		return _vmi_thread_stack_out_of_memory(t, ip);
	block->i64 = t->locals[block_index].i64;
	return ip + sizeof(vmi_instr_load);
}

const vm_byte* _vmi_thread_loadlx(vmi_thread* t, const vm_byte* ip)
{
	const vmi_instr_loadx* const instr = (const vmi_instr_loadx*)ip;
	vmi_stack_block* const block = vmi_stack_push(&t->stack, 1);
	if (block == NULL)
		return _vmi_thread_stack_out_of_memory(t, ip);
	block->i64 = t->locals[instr->block_index].i64;
	return ip + sizeof(vmi_instr_loadx);
}
...
inline const vm_byte* _vmi_thread_store(vmi_thread* t, const vm_byte* ip, const vm_uint32 block_index)
{
	...
	if (block == NULL)
		return _vmi_thread_stack_out_of_memory(t, ip);
	t->locals[block_index].value = block->value;
...
const vm_byte* _vmi_thread_storex(vmi_thread* t, const vm_byte* ip)
{
	...
	if (block == NULL)
		return _vmi_thread_stack_out_of_memory(t, ip);
	t->locals[instr->block_index].value = block->value;
	return ip + sizeof(vmi_instr_storex);
}

inline const vm_byte* _vmi_thread_storel(vmi_thread* t, const vm_byte* ip, const vm_uint32 block_index)
{
	const vmi_stack_block* const block = vmi_stack_pop(&t->stack, 1);
	if (block == NULL)
		return _vmi_thread_stack_out_of_memory(t, ip);
	t->locals[block_index].i64 = block->i64;
	return ip + sizeof(vmi_instr_store);
}

const vm_byte* _vmi_thread_storelx(vmi_thread* t, const vm_byte* ip)
{
	const vmi_instr_storex* const instr = (const vmi_instr_storex*)ip;
	const vmi_stack_block* const block = vmi_stack_pop(&t->stack, 1);
	if (block == NULL)
		return _vmi_thread_stack_out_of_memory(t, ip);
	t->locals[instr->block_index].i64 = block->i64;
	return ip + sizeof(vmi_instr_storex);
}
